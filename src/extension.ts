...
export async function activate(context: vscode.ExtensionContext) {
	...
	let targetName = getProjectTargetName();
	if(!(await targetName))
	{
	...
	const execPath = path.join(workspaceFolder.uri.fsPath, 'build', 'my_exe');
    ...
    let compileCommandsDisposable = vscode.commands.registerCommand('qtQmakeTasks.generateCompileCommands', () => {
        ...
        terminal.sendText('qmake -DCMAKE_EXPORT_COMPILE_COMMANDS=ON');
        terminal.sendText('make clean');
        ...
        }, 5000); // Wait a bit for compilation to complete
    });
...
async function getProjectTargetName(): Promise<string | null> {
	// Find .pro file in the workspace
	const proFiles = vscode.workspace.findFiles('**/*.pro', '**/build');
	
	if ((await proFiles).length > 0) {
		return parseProFileTargetName((await proFiles)[0].fsPath);
	}
